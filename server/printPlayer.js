...
function printDescrizione(nav_list)
{
    playersRef.doc(playerId).get().then((doc) => {
        if (doc.exists) {
            ...
            const privilegio_bg = data.privilegio_bg;
            const tratti_caratteriali = data.tratti_caratteriali;
            const ideali = data.ideali;
            const legami = data.legami;
            const difetti = data.difetti;

            nav_list.innerHTML = `<div class="spazio_inteno">
            <p style="padding: 20px; font-weight: bold; color: grey;">Tratti Caratteriali</p>
            <textarea disabled style="padding: 20px;">${tratti_caratteriali}</textarea><br>
            <p style="padding: 20px; font-weight: bold; color: grey;">Ideali</p>
            <textarea disabled style="padding: 20px;">${ideali}</textarea><br>
            <p style="padding: 20px; font-weight: bold; color: grey;">Legami</p>
            <textarea disabled style="padding: 20px;">${legami}</textarea><br>
            <p style="padding: 20px; font-weight: bold; color: grey;">Difetti</p>
            <textarea disabled style="padding: 20px;">${difetti}</textarea><br>
            <p style="padding: 20px; font-weight: bold; color: grey;">Privilegio BackGround</p>
            <textarea disabled style="padding: 20px;">${privilegio_bg}</textarea><br><br>
            </div>
            `;

            function updateTextareaHeight(textareaId) {
                const textarea = document.getElementById(textareaId);
                if (textarea) {
                    textarea.style.height = 'auto'; // Ripristina l'altezza predefinita
            ...
            document.getElementById('tratti_caratteriali').addEventListener('input', () => {
                updateTextareaHeight('tratti_caratteriali');
            });

            document.getElementById('ideali').addEventListener('input', () => {
                updateTextareaHeight('ideali');
            });

            document.getElementById('difetti').addEventListener('input', () => {
                updateTextareaHeight('difetti');
            });

            document.getElementById('legami').addEventListener('input', () => {
                updateTextareaHeight('legami');
            });

            document.getElementById('privilegi_bg').addEventListener('input', () => {
                updateTextareaHeight('privilegi_bg');
            });
